...
def main():
    ...
    parser.add_argument("--method", type=str, nargs="+", default=["conv"])  #
    parser.add_argument("--ndim", type=int, nargs="+", default=[1, 2, 3, 4])
    parser.add_argument("--loc", type=str, nargs="+", default=["cccc", "nnnn", "cnnn", "nccc"])
    ...
    for method in args.method:
        for ndim in args.ndim:
            for loc in {s[:ndim] for s in args.loc}:
                ...
                if isinstance(mod, odil.backend.ModTensorflow) and ndim == 4 and method == "conv":
                    continue
                try:
                ...
                print(
                    f"method={method:>5}, ndim={ndim}, loc={loc:>4}:", "{} {}".format(msg, "FAIL" if failed else "PASS")
                )
    exit(count_failed)
