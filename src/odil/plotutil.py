...
def apply_clip_box(ax, artists, lower=(0, 0), upper=(1, 1.02)):
    clipbox = matplotlib.transforms.TransformedBbox(matplotlib.transforms.Bbox([lower, upper]), ax.transAxes)
    for l in artists:
...
def set_log_ticks(xaxis):
    locmin = matplotlib.ticker.LogLocator(base=10.0, subs=np.arange(0.1, 0.99, 0.1), numticks=12)
