...
def transform_u(u, extra, mod):
    ...
    u = mod.concatenate([extra.u_init[None, :], u[1:-1], extra.u_final[None, :]], axis=0)
    return u
...
def parse_args():
    ...
    parser.add_argument("--c_diff", type=float, default=0.01, help="Diffusivity")  #
    parser.add_argument("--c_src", type=float, default=0.1, help="Uniform source")  #
    parser.add_argument("--c_vel", type=float, default=0.2, help="Advection velocity")  #
    odil.util.add_arguments(parser)
    odil.linsolver.add_arguments(parser)

    parser.set_defaults(frames=3, plot_every=50, report_every=50, history_every=10)
...
def plot_func(problem, state, epoch, frame, cbinfo=None):
    ...
    title = "epoch={:}, diff={:.3g}, src={:.3g}, vel={:.3g}".format(epoch, *coeff)
    umax = max(abs(np.max(ref_u)), abs(np.min(ref_u)))
    fig = odil.plot.plot_1d(
        domain,
        ref_u,
        state_u,
        cmap="RdBu_r",
...
def main():
    ...
    callback = odil.make_callback(
        problem, args, plot_func=plot_func, report_func=report_func, history_func=history_func
    )
