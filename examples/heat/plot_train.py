...
for key, name, ylbl in [
    ...
    for u, lbl, c, m in [
        ...
        if u is None:
            continue
        error = u[key]
        print("Last {} from {}: {:.6g} after {:.0f} iterations".format(key, lbl, error[-1], u["epoch"][-1]))
        ax.plot(u["epoch"], error, label=lbl, c=c)
    ax.set_xlabel("epoch")
    ax.set_ylabel(ylbl)
    ax.set_xscale("log")
    ax.set_yscale("log")
    ax.set_xticks(10.0 ** np.arange(0, 7))
    if name == "u":
        yticks = 10.0 ** np.arange(-3, 0.1)
    elif name == "k":
        yticks = 10.0 ** np.arange(-2, 1.1)
