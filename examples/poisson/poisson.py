...
def get_ref_rhs(name, args, domain):
    ...
    if name == "osc":
        ...
        x, y = xw
        fu = (
            ((-4 * k**4 * pi**2 * x**2) - pi**2) * sin(k**2 * pi * x**2) + 2 * k**2 * pi * cos(k**2 * pi * x**2)
        ) * sin(pi * y)
    else:
        raise ValueError("Unknown name=" + name)
    return fu
...
def operator(ctx):
    ...
    if args.mgloss:
        ...
        restrict = partial(odil.core.restrict_to_coarser, loc="c" * ndim, mod=mod)
...
def parse_args():
    ...
    parser.add_argument("--ndim", type=int, choices=[1, 2, 3, 4, 5, 6], default=2, help="Space dimension")
    parser.add_argument("--N", type=int, default=32, help="Grid size")
    parser.add_argument("--cellbased", type=int, default=1, help="Cell-based fields")
    parser.add_argument("--dump_xmf", type=int, default=0, help="Dump XMF+RAW files")
    parser.add_argument("--plot", type=int, default=0, help="Enable plotting")
    parser.add_argument("--ref", type=str, default="hat", choices=("hat", "osc"), help="Reference solution")
    parser.add_argument(
        "--rhs", type=str, default="discrete", choices=("discrete", "exact"), help="Reference right-hand side"
    )
    parser.add_argument("--osc_k", type=float, default=2, help="Parameter for ref='osc'")
    parser.add_argument("--mgloss", type=int, default=0, help="Use multigrid norm with mgloss terms in loss")
    odil.util.add_arguments(parser)
    odil.linsolver.add_arguments(parser)

    parser.set_defaults(frames=4, report_every=100, history_every=10, plot_every=100, history_full=50)
...
def make_problem(args):
    ...
    ndim = args.ndim
    domain = odil.Domain(
        cshape=[args.N] * ndim, dimnames=["x", "y", "z", "sx", "sy", "sz"][:ndim], multigrid=args.multigrid, dtype=dtype
    )
    if domain.multigrid:
