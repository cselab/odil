...
def plot(lines, args):
    for line in lines:
        if not os.path.isfile(line[0]):
            print('{} not found, skipping'.format(line[0]))
    lines = [line for line in lines if os.path.isfile(line[0])]
...
if __name__ == "__main__":
    args = parse_args()
    lines = [
        ('bfgs_684/train.csv', 'L-BFGS_TF 684 points', 'C0-'),
        ('bfgs_171/train.csv', 'L-BFGS_TF 171 points', 'C1-'),
